...
Level-shift and adjacent-offset statistics use robust model inliers, so isolated endpoint or reacquisition aliases do not reject an otherwise supported recording. By default, one or two consecutive model-outlier windows do not alone fail pair validation; they remain excluded from rendering anchors. Pair acceptance alone does not establish interpolation evidence. Rejected observations and robust-model outliers form the same low-confidence run for two-sided interpolation; a coarse alias inside a weak interval cannot become its supporting anchor. Unsupported, sustained model-outlier runs remain a separate discontinuity check and can only use the small-offset continuity allowance when their measured offsets stay within the analysis tolerance.
...
Clock-continuity checks convert the configured millisecond allowance directly to samples: the default 1 ms is 20 samples at 20 kHz and 1 sample at 1 kHz. They do not inherit the legacy four-sample minimum or integer rounding used by post-merge QC.

Final alignment QC uses the synchronization high-pass cutoff (200 Hz by default), a causal second-order filter with valid warm-up, and up to the configured observation-window duration (10 seconds by default). The production tolerance at 20 kHz remains 1 ms. A requested window too short for the lag search yields an unavailable measurement and WARN without excluding data; the window is not enlarged or the lag search clipped to manufacture support. Repeated-failure, recovery, and residual-correction decisions require non-overlapping measurement windows for independent support. `alignment_quality.dat` records timing warnings separately from structural validity and does not zero measured waveforms. Direct callers of `validate_segment_staged_merge` should pass `highpass_hz` to use this feature definition; omission retains the unfiltered direct-call behavior.
